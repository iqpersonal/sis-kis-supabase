...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (MANAGE_ACTIONS.includes(action) && !hasPermission(auth.role, "general_store.manage", auth.secondaryRoles)) {
      return NextResponse.json({ error: "Forbidden: insufficient permissions" }, { status: 403 });
    }
    if (REQUEST_ACTIONS.includes(action) && !hasPermission(auth.role, "general_store.request", auth.secondaryRoles)) {
      return NextResponse.json({ error: "Forbidden: insufficient permissions" }, { status: 403 });
    }
    ...
    const handlers: Record<string, () => Promise<{ data?: unknown; error?: string; status?: number }>> = {
      ...
      transfer_out: () => transferItems(supabase, cfg, IT_STORE_CONFIG, body, auth.uid),
    };
